...
namespace ESCPOS_NET.DataValidation
{
        ...
        public void Validate(T type, string data, BarcodeCode? code = null)
        {
            ...
            if (constraints.ValidLengths != null)
            {
                if (!constraints.ValidLengths.Contains(data.Length))
                    throw new ArgumentException(
                        $"Code '{data}' is not a valid length from: [{string.Join(", ", constraints.ValidLengths)}].");
            }
            ...
            if (constraints.ValidChars == "7BIT-ASCII")
            {
                if (!data.All(x => x <= 127 && x >= 0))
                    throw new ArgumentException(
                        $"Code '{data}' contained invalid characters not in: {constraints.ValidChars}.");
            }
    ...
    {
        private static BarcodeDataValidator singletonBarcode;
        private static TwoDimensionCodeDataValidator singleton2DCode;

        public static void ValidateBarcode(BarcodeType type, BarcodeCode code, string data)
        {
            if (singletonBarcode is null) singletonBarcode = new BarcodeDataValidator();
        ...
        {
            if (singleton2DCode is null) singleton2DCode = new TwoDimensionCodeDataValidator();
        ...
        private class BarcodeDataValidator : BaseDataValidator<BarcodeType>
        {
            public BarcodeDataValidator()
            {
                _constraints = new Dictionary<BarcodeType, DataConstraint>
                {
                    ...
                    {
                        BarcodeType.UPC_E,
                        new DataConstraint {ValidLengths = new List<int> {6, 7, 8, 11, 12}, ValidChars = "0123456789"}
                    },
                    {
                        BarcodeType.JAN13_EAN13,
                        new DataConstraint {MinLength = 12, MaxLength = 13, ValidChars = "0123456789"}
                    },
                    {
                        BarcodeType.JAN8_EAN8,
                        new DataConstraint {MinLength = 7, MaxLength = 8, ValidChars = "0123456789"}
                    },
                    {
                        BarcodeType.CODE39,
                        new DataConstraint
                        {
                            MinLength = 1, MaxLength = 255, ValidChars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./"
                        }
                    },
                    {BarcodeType.ITF, new DataConstraint {MinLength = 2, MaxLength = 255, ValidChars = "0123456789"}},
                    {
                        BarcodeType.CODABAR_NW_7,
                        new DataConstraint {MinLength = 2, MaxLength = 255, ValidChars = "0123456789ABCDabcd$+-./:"}
                    },
                    {
                        BarcodeType.CODE93,
                        new DataConstraint {MinLength = 1, MaxLength = 255, ValidChars = "7BIT-ASCII"}
                    },
                    {
                        BarcodeType.CODE128,
                        new DataConstraint {MinLength = 1, MaxLength = 253, ValidChars = "7BIT-ASCII"}
                    },
                    {
                        BarcodeType.GS1_128,
                        new DataConstraint {MinLength = 1, MaxLength = 253, ValidChars = "7BIT-ASCII"}
                    },
                    {
                        ...
                        new DataConstraint
                        {
                            MinLength = 13, MaxLength = 13, ValidChars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./"
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        new DataConstraint
                            {MinLength = 2, MaxLength = 255, ValidChars = "0123456789ABCDabcd !\"%$'()*+,-./:;<=>?_{"}
                    }
                };
            }
                ...
                switch (type)
                {
                    case BarcodeType.UPC_E:
                        if (barcode.Length != 6 && !barcode.StartsWith("0", StringComparison.InvariantCulture))
                            throw new ArgumentException(
                                $"UPC_E Barcode {barcode} with length of 7, 8, 11, or 12 must start with 0.");
                    ...
                    case BarcodeType.ITF:
                        if (barcode.Length % 2 != 0)
                            throw new ArgumentException(
                                $"ITF Barcode {barcode} has length {barcode.Length}, which is not an even number.");
                    ...
                    case BarcodeType.CODABAR_NW_7:
                        if (!"ABCD".Contains(barcode[0]) || !"ABCD".Contains(barcode[barcode.Length - 1]))
                            throw new ArgumentException(
                                $"CODABAR_NW_7 Barcode {barcode} must start and end with an ABCD character.");

                        if (barcode.Skip(1).Take(barcode.Length - 2).Any(x => "ABCD".Contains(x)))
                            throw new ArgumentException(
                                $"CODABAR_NW_7 Barcode {barcode} must not include ABCD characters in the body of the barcode.");

                        break;

                    case BarcodeType.CODE93:
                        if (!barcode.StartsWith("*", StringComparison.InvariantCulture) ||
                            !barcode.EndsWith("*", StringComparison.InvariantCulture))
                            throw new ArgumentException(
                                $"CODE39 Barcode {barcode} must start and end with * characters.");
                    ...
                    case BarcodeType.CODE128:
                        if (code == BarcodeCode.CODE_C)
                        {
                            ...
                            if (!barcode.All(x => x <= '9' && x >= '0'))
                                throw new ArgumentException(
                                    $"Barcode {barcode} is invalid.  CODE128 CODE_C barcodes only support numeric characters.");
                        }
